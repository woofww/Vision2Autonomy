...
def test_interactive_site_has_required_labs() -> None:
    parser = parse_site()
    assert {"convolution-lab", "canny-lab", "harris-lab", "matching-lab", "stereo-lab", "hough-lab", "flow-lab", "calib-lab", "pnp-lab", "vo-lab"} <= parser.ids
...
def test_javascript_contains_all_interactions() -> None:
    ...
    for feature in ("calculateConvolution", "cannyStages", "renderRegion", "renderRansacThreshold", "renderStereoDepth", "themeToggle", "setPointerCapture", "scrollLeft", "scrollMatrix", "requestAnimationFrame", 'pointerType !== "mouse"', "houghVote", "houghPeaks", "renderHough", "computeFlowLite", "renderFlowLab", "drawArrow", "renderPnPLab", "projectPnPPoint", "renderVODriftLab", "buildDriftTrajectory"):
