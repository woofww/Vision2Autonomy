...
## 自动驾驶中的作用：寻找车道、路缘和目标轮廓

![计算机视觉算法在自动驾驶场景中的对应关系](../../docs/assets/autonomous_driving_cv_map.png)

图中的青色细线对应梯度明显的结构：车道线两侧、路缘、前车轮廓和行人外形。Canny 能为传统车道检测、轮廓分析和标定板提取提供稀疏结构，但“检测到边缘”不等于“理解了对象”。阴影、护栏和路面裂纹同样会产生强边缘，所以后续必须结合几何、语义或时间信息。

**场景例子：** 在结构清晰的高速公路上，边缘可用于车道候选；在树影密集的城市道路上，仅凭边缘会产生大量干扰。
